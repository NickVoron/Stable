...
namespace dx11
{
	...
	int createTextureWithMip(ID3D11Device* device, const Plane< Color<fmt> >& plane, ID3D11Texture2D** texture, int mipsCount)
	{
		...
		//image::filter::kernel2Ext<image::filter::Sharpen3> kernel;
		//kernel.normalize();
		//int countMips = filter::generateMipLevels(plane, mips, mipsCount, kernel);
	  ...
	  /*
	template<Format fmt>
	void createTextureWithMipmap(ID3D11Device* device, const Plane< Color<fmt> >& plane, int mipLevels, ID3D11Texture2D** texture)
	{
		ENFORCE(device);
		ENFORCE(texture);
...
// 		context->UpdateSubresource(texture, 0, NULL, plane.bmp, plane.pitchInBytes, NULL);
	}
	*/
