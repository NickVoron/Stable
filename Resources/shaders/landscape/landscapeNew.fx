...
LandVrtPos calcLandVertexPos(float2 inPos, float2 inWPos, float4 composedOriginAndRenderSize, float level, float4 lodCornerGamma, float2 oddFlag)
{
	...
	Out.worldPos.y = calcHeight(texCoord, pointGamma)/2;
	//Out.worldPos.y = 0;
...
//	tsN = N;
	...
	float4 diffColor = (sunColor * clouds + specValue * sunColor) * dotNL * shadow + ambient;
	//float4 diffColor = dotNL* shadow + ambient;
