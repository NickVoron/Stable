...
VS_OUTPUT MainVS( VS_INPUT In)
{
	...
	float3 WPos;
	WPos.xy = unpack2(f0.a) * 16.0f*shift.w;
	
	float2 zSI = unpack2(f0.r);
	WPos.z = zSI.x * 16.0f*shift.w;
	float tidx;
	float size = modf(zSI.y*16.0f, tidx)*4.0f*shift.w;
	...
	//float3 Pp = mul( float4( WPos, 1 ), (float4x3)View ) + In.Pos*size;
   	//Out.Pos  = mul( float4( Pp, 1 ), Projection );
	...
	float3 lightPos = wp + 300*float3(1,0.4,1) + lightDir.xyz*400;
	//Out.SH *= 100000.0f/pow(length(lightPos - WPos), 2.0f);
	Out.SH = 200.0f/pow(length(lightPos - WPos), 1.0f);
