...
ShadowReceiveOutput ShadowReceiveVS(ShadowReceiveInput In)
{
	...
	Out.Position.xyw = In.Position.xyw * ShadowClipZW.y;
	Out.Position.z = ShadowClipZW.x;
...
float SampleShadowPCF3x3_NV(float4 shadowCoords, sampler sampShadowDepth, int2 ShadowSize, float4 ShadowTexelUV)
{
...
{
	return SampleShadowPCF3x3_NV(shadowCoords, sampShadowDepth, ShadowSize, ShadowTexelUV);

	//NV - hardware PCF when using Linear texture filtering
	return tex2Dproj(sampShadowDepth, shadowCoords).r;
}

float SampleShadowPCF3x3_ATI(float4 shadowCoords, sampler sampShadowDepth, int2 ShadowSize, float4 ShadowTexelUV)
{
	float2 shadowUV = shadowCoords.xy / shadowCoords.w;
	...
	percentInLight  = dot(step(shadowDepth, shadowMapVals[0]), shadowMapWeights[0]);
	percentInLight += dot(step(shadowDepth, shadowMapVals[1]), shadowMapWeights[1]);
	percentInLight += dot(step(shadowDepth, shadowMapVals[2]), shadowMapWeights[2]);
	percentInLight += dot(step(shadowDepth, shadowMapVals[3]), shadowMapWeights[3]);
	
	return percentInLight * (1.0f / 16);
}

float SampleShadowPCF_ATI(float4 shadowCoords, sampler sampShadowDepth, int2 ShadowSize, float4 ShadowTexelUV)
{
	return SampleShadowPCF3x3_ATI(shadowCoords, sampShadowDepth, ShadowSize, ShadowTexelUV);
...
float4 ShadowReceivePS(uniform bool hasATI, ShadowReceiveOutput In) : COLOR
{
	...
	viewPos.xyz *= tex2D(sampScreenDepth, In.ScreenUV).r;
	...
	float lightAmountDyn = hasATI
		? SampleShadowPCF_ATI(mul(viewPos, DynamicShadowTexTM), sampDynamicShadowDepth_Point, DynamicShadowSize, DynamicShadowTexelUV)
		: SampleShadowPCF_NV(mul(viewPos, DynamicShadowTexTM), sampDynamicShadowDepth_Linear, DynamicShadowSize, DynamicShadowTexelUV);
	...
	if (bInFrustrum)
	{
		float2 lastUV = lastViewProjCoords.xy / lastViewProjCoords.w;
#ifdef D3D9_RASTER_OFFSET
		lastUV += ScreenTexelUV * 0.5f;
#endif

		float depth_Last = tex2D(sampScreenLastDepth, lastUV).r;
		if (lastViewProjCoords.w < depth_Last + 0.1f)
		{
			float4 shadowVal_Last = tex2D(sampScreenLastShadow, lastUV);
			float lightAmount_Last = shadowVal_Last.g;
			float shadowConf_Last = shadowVal_Last.b;
			
			float weight = pow(abs(shadowConf), lerp(0.0f, 5.0f, shadowConf_Last));
...
float4 ShadowReceiveDynPS(uniform bool hasATI, ShadowReceiveOutput In) : COLOR
{
	...
	float lightAmount = hasATI
		? SampleShadowPCF_ATI(mul(viewPos, StaticShadowTexTM), sampStaticShadowDepth_Point, StaticShadowSize, StaticShadowTexelUV)
		: SampleShadowPCF_NV(mul(viewPos, StaticShadowTexTM), sampStaticShadowDepth_Linear, StaticShadowSize, StaticShadowTexelUV);
