...
Texture2D    colorDownsampled4xTex	: register(t2);
Texture2D    depthDownsampled4xTex	: register(t3);

SamplerState smpl			: register(s0);


float4 color(float2 uv)		{ return colorTex.SampleLevel(smpl, uv, 0); }
float4 depth(float2 uv)		{ return depthTex.Sample(smpl, uv); }
float4 color4X(float2 uv)	{ return colorDownsampled4xTex.Sample(smpl, uv); }
float4 depth4X(float2 uv)	{ return depthDownsampled4xTex.Sample(smpl, uv); }
