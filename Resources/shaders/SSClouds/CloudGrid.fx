...
void mainVS(out SVSOutput _Output, in SVSInput _Input)
{
	...
	// texture coordinate 
	_Output.vTex.xy = _Input.vPos.zw * vUVParam.xy + vUVParam.zw;
	_Output.vTex.z = vWorldPos.y;
}
...
float4 mainPS(in SPSInput _Input) : COLOR0
{
	...
	float a = saturate((_Input.vTex.z - fade.x) / (fade.y - fade.x));
	return _fDensity * a;
}
